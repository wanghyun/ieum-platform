...
function ieum_character_print_expected_weeks($month)
{
...
$expected_weeks = ieum_character_print_expected_weeks($month);
...
$student_result = sql_query("
    select s.student_id, s.student_code, s.student_name, s.birth_date, s.admission_date, s.attendance_days,
           s.program_code, s.grade_group, s.school_name, c.class_name, c.start_time
...
while ($row = sql_fetch_array($student_result)) {
    $score = ieum_character_month_score($academy_id, $row, $month);
    if (!$score || !empty($score['is_before_admission']) || (int) $score['evaluated_weeks'] < $expected_weeks) {
        continue;
    }
    $students[] = $row;
}

$back_url = IEUM_URL . '/admin/character_report.php?' . http_build_query(array(
    'month' => $month,
...
<header class="toolbar">
    <div>
        <h1>인성 리포트 전체 인쇄</h1>
        <div class="meta"><?php echo get_text($academy['academy_name']); ?> · <?php echo get_text($month); ?> · 인쇄 준비 원생 <?php echo number_format(count($students)); ?>명</div>
    </div>
    <div class="actions">
...
<?php if (!$students) { ?>
<section class="empty">인쇄할 인성 리포트가 없습니다. <?php echo number_format($expected_weeks); ?>주차 인성 입력을 완료한 학생만 전체 인쇄 대상입니다.</section>
<?php } else { ?>
<main class="bundle">
    <?php foreach ($students as $index => $student) {
        $url = IEUM_URL . '/admin/character_parent_report.php?' . http_build_query(array(
            'month' => $month,
            'student_id' => (int) $student['student_id'],
            'print_bundle' => 1,
        ));
    ?>
    <div class="preview-label"><?php echo number_format($index + 1); ?> / <?php echo number_format(count($students)); ?> · <?php echo get_text($student['student_name']); ?></div>
    <section class="sheet-preview">
        <iframe class="sheet-frame" src="<?php echo get_text($url); ?>" title="<?php echo get_text($student['student_name']); ?> 인성 리포트" scrolling="no"></iframe>
    </section>
    <?php } ?>
</main>
<?php } ?>
